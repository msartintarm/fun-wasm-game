...
import { TRACKS, type MusicTrackId } from "./tracks";
...
function loadMidi(id: MusicTrackId): Promise<Midi> {
  ...
  if (!cached) {
    cached = Midi.fromUrl(TRACKS[id].midiUrl);
    parsedCache.set(id, cached);
  }
  return cached;
}

export function createMidiAudioEngine(): AudioEngine {
  ...
  return {
    resume() {
      if (contextStarted) return; // already resuming/resumed — idempotent
    ...
    startTrack(id) {
      ...
      if (!hasPendingTrackRequest) return;
      // If resume() hasn't been called yet, contextStarted is still null
      // and this is a no-op for now — resume()'s own .then() will flush
      // the pending request once a real gesture actually happens. If
      // resume() already ran (whether or not it's resolved yet), attach to
      // that same promise instead of guessing based on a boolean.
