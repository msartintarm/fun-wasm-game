...
// Set at build time only (e.g. NEXT_PUBLIC_BASE_PATH=/snake in the GitHub
// Pages deploy workflow) — empty locally, so `npm run dev`/`npm run build`
// behave exactly as before unless this is explicitly set. Read the same
// way in GameCanvas.tsx, wherever a raw absolute path (not next/link or
// next/image, which basePath rewrites automatically) needs the prefix too.
